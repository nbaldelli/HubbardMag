...
function currents_from_correlation_V2(t, lattice, C; α=0)
...
let
    t = 1; tp = 0.2; J = 0.4; U=(4*t^2)/J; α=1/60; doping = 1/16; max_linkdim = 1200
    Nx = 32; Ny = 4
    ...
    h5open("corr.h5","r") do f
        ...
        plt.title("Nx=$Nx, Ny=$Ny, α=$α, maxlinkdim=$maxlinkdim")
        ...
        couples, curr_plot = currents_from_correlation_V2(t, lat, corr_dens, α=α) #current
        fig, ax = plt.subplots(1, dpi = 150)
        ax.set_ylim(0.5,Ny+0.5)
